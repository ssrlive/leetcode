...
impl Solution {
    pub fn reverse_k_group<T>(head: Option<Box<ListNode<T>>>, k: i32) -> Option<Box<ListNode<T>>> {
        pub fn _reverse_k_group<T>(
...
fn test_reverse_k_group() {
    ...
    assert_eq!(
        Solution::reverse_k_group(ListNode::<i32>::from_vec(&[]), 1),
        ListNode::from_vec(&[])
    );
}
