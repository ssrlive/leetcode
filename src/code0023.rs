...
impl Solution {
    pub fn merge_k_lists<T: std::cmp::PartialOrd>(
        ...
        loop {
            let mut min: Option<&Box<ListNode<T>>> = None;
            let mut min_index = 0;
    ...
    pub fn merge_k_lists_2<T: Ord + Copy>(
        lists: Vec<Option<Box<ListNode<T>>>>,
    ) -> Option<Box<ListNode<T>>> {
...
fn test_merge_k_lists() {
    ...
    let lists: Vec<Option<Box<ListNode<i32>>>> = vec![];
    let result = None;
    assert_eq!(Solution::merge_k_lists_2(lists), result);

    let lists: Vec<Option<Box<ListNode<i32>>>> = vec![None];
    let result = None;
    assert_eq!(Solution::merge_k_lists_2(lists), result);
}
