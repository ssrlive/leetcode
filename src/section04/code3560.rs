...
//
// You may cut the logs into smaller pieces, where the cost of cutting a log of length x into logs of
// length len1 and len2 is cost = len1 * len2 such that len1 + len2 = x.
//
...
impl Solution {
    pub fn min_cutting_cost(n: i32, m: i32, k: i32) -> i64 {
        ...
        if m > k && n <= k {
            ans += (m - k) as i64 * k as i64;
        }
        if n > k && m <= k {
            ans += (n - k) as i64 * k as i64;
        }
