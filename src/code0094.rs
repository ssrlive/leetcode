...
use std::rc::Rc;
impl Solution {
    pub fn inorder_traversal<T: Copy>(root: Option<Rc<RefCell<TreeNode<T>>>>) -> Vec<T> {
        fn inorder_traversal_recursive<T: Copy>(
            ...
            if let Some(node) = root {
                ...
                inorder_traversal_recursive(&node.borrow().right, result);
            }
        }

        let mut result = Vec::new();
        inorder_traversal_recursive(&root, &mut result);
        result
    }
}
