...
fn test() {
    ...
    let res = vec![vec![1, 4, 14], vec![4, 7, 16]];
    assert_eq!(Solution::split_painting(segments), res);

    let segments = vec![vec![1, 7, 9], vec![6, 8, 15], vec![8, 10, 7]];
    let res = vec![vec![1, 6, 9], vec![6, 7, 24], vec![7, 8, 15], vec![8, 10, 7]];
    assert_eq!(Solution::split_painting(segments), res);

    let segments = vec![vec![1, 4, 5], vec![1, 4, 7], vec![4, 7, 1], vec![4, 7, 11]];
    let res = vec![vec![1, 4, 12], vec![4, 7, 12]];
    assert_eq!(Solution::split_painting(segments), res);
}
