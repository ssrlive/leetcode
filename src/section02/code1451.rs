...
fn test() {
    let cases = vec![
        ("Leetcode is cool".to_string(), "Is cool leetcode".to_string()),
        ("Keep calm and code on".to_string(), "On and keep calm code".to_string()),
        ("To be or not to be".to_string(), "To be or to be not".to_string()),
    ];
    for (text, expected) in cases {
        assert_eq!(Solution::arrange_words(text), expected);
    }
}
