...
fn test() {
    let cases = vec![
        ...
        (4, 2, vec![3, 3, -1, 2], vec![0, 0, 162, 914], 1076),
    ];
    for (n, head_id, manager, inform_time, expect) in cases {
        assert_eq!(Solution::num_of_minutes(n, head_id, manager, inform_time), expect);
    }
}
