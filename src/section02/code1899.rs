...
fn test() {
    let cases = vec![
        (vec![vec![2, 5, 3], vec![1, 8, 4], vec![1, 7, 5]], vec![2, 7, 5], true),
        (vec![vec![3, 4, 5], vec![4, 5, 6]], vec![3, 2, 5], false),
        (
    ...
    for (triplets, target, expected) in cases {
        assert_eq!(Solution::merge_triplets(triplets, target), expected);
    }
}
