...
fn test() {
    let cases = vec![
        (vec![1, 3], 3, vec!["Push", "Push", "Pop", "Push"]),
        (vec![1, 2, 3], 3, vec!["Push", "Push", "Push"]),
        (vec![1, 2], 4, vec!["Push", "Push"]),
    ];
    for (target, n, expected) in cases {
        assert_eq!(Solution::build_array(target, n), expected);
    }
}
