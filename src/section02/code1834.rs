...
impl Solution {
    pub fn get_order(tasks: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        indices.sort_unstable_by_key(|&i| -tasks[i][0]);
        let mut heap: BinaryHeap<Reverse<(i32, usize)>> = BinaryHeap::new();
        let mut ans = Vec::with_capacity(tasks.len());
        let mut time = 0;
        loop {
            match heap.pop() {
                Some(Reverse((duration, i))) => {
                    ans.push(i as i32);
                    time += duration;
                }
                None => match indices.last() {
                    Some(&j) => time = tasks[j][0],
                    None => break ans,
                },
            }
            while !indices.is_empty() && tasks[*indices.last().unwrap()][0] <= time {
...
fn test() {
    let cases = vec![
        (vec![vec![1, 2], vec![2, 4], vec![3, 2], vec![4, 1]], vec![0, 2, 3, 1]),
        (
