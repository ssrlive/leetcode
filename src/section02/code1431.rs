...
fn test() {
    let cases = vec![
        ...
        (vec![4, 2, 1, 1, 2], 1, vec![true, false, false, false, false]),
        (vec![12, 1, 12], 10, vec![true, false, true]),
    ];
    for (candies, extra_candies, expected) in cases {
        assert_eq!(Solution::kids_with_candies(candies, extra_candies), expected);
    }
}
