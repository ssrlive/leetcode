...
impl Solution {
    pub fn create_target_array(nums: Vec<i32>, index: Vec<i32>) -> Vec<i32> {
        ...
        let f = |(x, ind): (&i32, &i32)| res.insert(*ind as usize, *x);
        nums.iter().zip(index.iter()).for_each(f);
        res
    }
...
fn test() {
    ...
    for (nums, index, expected) in cases {
        assert_eq!(Solution::create_target_array(nums, index), expected);
    }
}
