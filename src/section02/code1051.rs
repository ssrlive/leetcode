...
// line in non-decreasing order by height. Let this ordering be represented by the integer array
...
impl Solution {
    pub fn height_checker(heights: Vec<i32>) -> i32 {
        ...
        heights.iter().zip(heights_sort.iter()).filter(|(h, hs)| h != hs).count() as i32
    }
}
