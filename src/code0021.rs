...
impl Solution {
    pub fn merge_two_lists<T: Default + PartialOrd + Copy>(
        list1: Option<Box<ListNode<T>>>,
        list2: Option<Box<ListNode<T>>>,
    ) -> Option<Box<ListNode<T>>> {
        let mut dummy = Some(Box::new(ListNode::<T>::new(T::default())));
    ...
    pub fn merge_two_lists_2<T: std::cmp::PartialOrd>(
        list1: Option<Box<ListNode<T>>>,
        list2: Option<Box<ListNode<T>>>,
    ) -> Option<Box<ListNode<T>>> {
        fn _merge_two_lists<T: std::cmp::PartialOrd>(
            list1: Option<Box<ListNode<T>>>,
            list2: Option<Box<ListNode<T>>>,
        ) -> Option<Box<ListNode<T>>> {
            match (list1, list2) {
...
fn test() {
    ...
    let l1: Option<Box<ListNode<i32>>> = ListNode::from_vec(&vec![]);
