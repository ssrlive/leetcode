...
impl Solution {
    pub fn diagonal_prime(nums: Vec<Vec<i32>>) -> i32 {
        ...
        let mut p = 0;
        for i in 0..nums.len() {
            p = p.max(is_prime(nums[i][i]) as i32 * nums[i][i]);
            p = p.max(is_prime(nums[i][nums.len() - i - 1]) as i32 * nums[i][nums.len() - i - 1]);
        }
        p
    }
}
...
#[test]
fn test() {
    let nums = vec![vec![1, 2, 3], vec![5, 6, 7], vec![9, 10, 11]];
    assert_eq!(Solution::diagonal_prime(nums), 11);
    let nums = vec![vec![1, 2, 3], vec![5, 17, 7], vec![9, 11, 10]];
    assert_eq!(Solution::diagonal_prime(nums), 17);
}
