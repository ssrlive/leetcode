...
struct FoodRatings {
    ...
    cuisines: Vec<String>,
    ratings: Vec<i32>,
    food_index: HashMap<String, usize>,
    rate_lists: HashMap<String, BTreeSet<(usize, usize)>>,
}
...
impl FoodRatings {
    ...
    fn highest_rated(&mut self, cuisine: String) -> String {
        if let Some(s) = self.rate_lists.get_mut(&cuisine) {
            ...
            for a in s.iter().rev() {
                ...
                len = a.0;
                v.push(self.foods[a.1].clone());
            }
            v.sort();
            return v[0].clone();
        }
        panic!["cuisine not found!"]
    }
}
...
fn test() {
    ...
    assert_eq!(food_ratings.highest_rated("korean".to_string()), "kimchi".to_string());
    assert_eq!(food_ratings.highest_rated("japanese".to_string()), "ramen".to_string());
    food_ratings.change_rating("sushi".to_string(), 16);
    assert_eq!(food_ratings.highest_rated("japanese".to_string()), "sushi".to_string());
