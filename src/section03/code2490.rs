...
impl Solution {
    pub fn is_circular_sentence(sentence: String) -> bool {
        fn _is_circular_sentence(sentence: String) -> Option<bool> {
            let words = sentence.split_ascii_whitespace();
            let mut chars = sentence.chars();
            let v = chars.clone().next()? == chars.next_back()?
                && words
                    .clone()
                    .zip(words.skip(1))
        ...
        _is_circular_sentence(sentence).unwrap_or(false)
    }
}
...
fn test() {
    ...
    let ans = true;
    assert_eq!(Solution::is_circular_sentence(sentence), ans);
    ...
    let sentence = "MuFoevIXCZzrpXeRmTssj lYSW U jM".to_string();
    let ans = false;
    assert_eq!(Solution::is_circular_sentence(sentence), ans);
}
