...
// the character 'c' to 'b' at a cost of 2, for a total cost of 1 + 2 = 3.
...
impl Solution {
    pub fn minimum_cost(source: String, target: String, original: Vec<char>, changed: Vec<char>, cost: Vec<i32>) -> i64 {
        let mut cc = vec![vec![i32::MAX as u64; 26]; 26];
        for i in 0..original.len() {
            let c1 = original[i] as usize - 97;
            let c2 = changed[i] as usize - 97;
            cc[c1][c2] = cc[c1][c2].min(cost[i] as u64);
        }
        ...
        for i in 0..source.len() {
            if target[i] != source[i] {
                let v = cc[source[i] as usize - 97][target[i] as usize - 97];
                if v >= i32::MAX as u64 {
                    return -1;
                }
                res += v;
...
fn test() {
    ...
    let res = 28;
    assert_eq!(Solution::minimum_cost(source, target, original, changed, cost), res);
