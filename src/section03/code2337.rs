...
impl Solution {
    pub fn can_change(start: String, target: String) -> bool {
        ...
        let mut sk1: Vec<(char, usize)> = Vec::new();
        let mut sk2: Vec<(char, usize)> = Vec::new();
        for i in 0..start.len() {
            ...
            if sk1.len() < sk2.len() && sk2[sk2.len() - 1].0 == 'R' {
                return false;
            }
            if sk1.len() > sk2.len() && sk1[sk1.len() - 1].0 == 'L' {
                return false;
            }
            if sk1.is_empty() && sk2.is_empty() {
                continue;
            }
            if sk1.len() == sk2.len() && sk1[sk1.len() - 1].0 != sk2[sk2.len() - 1].0 {
                return false;
            }
        }
        sk1.len() == sk2.len()
