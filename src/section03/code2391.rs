...
fn test() {
    ...
    for (garbage, travel, expected) in cases {
        let garbage = garbage.into_iter().map(|s| s.to_string()).collect();
        assert_eq!(Solution::garbage_collection(garbage, travel), expected);
    }
}
