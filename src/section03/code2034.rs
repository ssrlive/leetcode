...
struct StockPrice {
    ...
    min: BinaryHeap<Reverse<(i32, i32)>>,
    max: BinaryHeap<(i32, i32)>,
}
...
impl StockPrice {
    ...
    fn update(&mut self, timestamp: i32, price: i32) {
        self.latest_ts = self.latest_ts.max(timestamp);
        self.prices.insert(timestamp, price);
        self.min.push(Reverse((price, timestamp)));
        self.max.push((price, timestamp));
    }
    ...
    fn maximum(&mut self) -> i32 {
        let mut peek = *self.max.peek().unwrap();
        while *self.prices.get(&peek.1).unwrap() != peek.0 {
            self.max.pop();
            peek = *self.max.peek().unwrap();
        }
    ...
    fn minimum(&mut self) -> i32 {
        let Reverse(mut peek) = *self.min.peek().unwrap();
        while *self.prices.get(&peek.1).unwrap() != peek.0 {
            self.min.pop();
            peek = self.min.peek().unwrap().0;
        }
