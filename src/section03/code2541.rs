...
impl Solution {
    pub fn min_operations(nums1: Vec<i32>, nums2: Vec<i32>, k: i32) -> i64 {
        let s1 = nums1.iter().map(|&a| a as i64).sum::<i64>();
        let s2 = nums2.iter().map(|&a| a as i64).sum::<i64>();
        if s1 != s2 {
            return -1;
        }
        ...
        for i in 0..nums1.len() {
            ...
            ret += (nums1[i] as i64 - nums2[i] as i64) / k as i64;
        }
...
fn test() {
    ...
    for (nums1, nums2, k, expected) in cases {
        assert_eq!(Solution::min_operations(nums1, nums2, k), expected);
    }
}
