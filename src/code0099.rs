...
use std::rc::Rc;
impl Solution {
    pub fn recover_tree<T: std::cmp::PartialOrd + Copy>(
        root: &mut Option<Rc<RefCell<TreeNode<T>>>>,
    ) {
        fn inorder<T: Copy>(
            n: Option<Rc<RefCell<TreeNode<T>>>>,
            v: &mut Vec<Rc<RefCell<TreeNode<T>>>>,
        ) {
            if let Some(n) = n {
    ...
    pub fn recover_tree_2<T: std::cmp::PartialOrd>(root: &mut Option<Rc<RefCell<TreeNode<T>>>>) {
        let mut stack = vec![];
        let mut curr = root.clone();
        let mut x = None;
        let mut y = None;
        let mut pred: Option<Rc<RefCell<TreeNode<T>>>> = None;
