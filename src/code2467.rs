...
//     if the price to open the gate is c, then both Alice and Bob pay c / 2 each. Similarly,
...
fn test() {
    ...
    assert_eq!(Solution::most_profitable_path(edges, 3, vec![-2, 4, 2, -4, 6]), 6);

    let edges = vec![vec![0, 1]];
    assert_eq!(Solution::most_profitable_path(edges, 1, vec![-7280, 2350]), -7280);
}
