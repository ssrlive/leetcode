...
use std::rc::Rc;
impl Solution {
    pub fn is_valid_bst<T: std::cmp::PartialOrd>(root: Option<Rc<RefCell<TreeNode<T>>>>) -> bool {
        fn validate<T: std::cmp::PartialOrd>(
            n: Option<Rc<RefCell<TreeNode<T>>>>,
            min: Option<Rc<RefCell<TreeNode<T>>>>,
            max: Option<Rc<RefCell<TreeNode<T>>>>,
        ) -> bool {
            match n {
                None => true,
                Some(n) => {
